...
class PQueuePriorityQueue(PQueue):
    """
        ...
        Performs a deprioritisation by marking items with priority > min_del_priority as deleted
        and ignoring them when they are extracted from the PQ.
    """
    ...
    def remove(self, item: object) -> None:
        """
            Remove (virtually) an item from the priority queue by setting the minimum seen priority to -inf
            (Not actually removing it from the queue).
    ...
    def update(self, item: object, ext_priority: float) -> bool:
        """
            Update the priority of an item in the priority queue.
            ...
            Args:
                item: the object to be updated
                priority: the new external priority of the item
        """
        if item not in self.deleted: 
            #print("Error, trying to update an item that is not in the deleted set")  
            return False

        # read the current priority of the item (the minimum seen)
        old_priority = self.deleted[item]["priority"]

        # check if new priority is lower than the current one
        if self._external_priority(old_priority) > ext_priority:
            self._mark_deleted(item, ext_priority) # update priority as the new one and mark as deleted all greater priorities
            self.queue.put((self._internal_priority(ext_priority), item)) # add new item to the queue
    ...
    def get(self) -> tuple:
        ...
        while not self.queue.empty(): # process the queue until it is empty
            ...
            if priority > min_del_priority: # extracted item is marked as deleted
                self.num_deleted -= 1
                continue
    ...
    def _mark_deleted(self, item: object, priority: float) -> None:
        ...
        if priority > self._external_priority(old_priority):
            print(f"Error, (inside mark_deleted) trying to mark an item as deleted with a priority higher than the current one. old_priority={old_priority}, priority={priority}")
            return
    ...
    def _cleanup_deleted(self) -> None:
        ...
        while self.enqueued() > 0: #while not self.queue.empty():
