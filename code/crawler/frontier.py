...
class QualityFrontierManager(FrontierManager):
    ...
    def __init__(self, updates = False, verbose: bool = True, oracle: bool = False) -> None:
        ...
        self.queue = PQueuePriorityQueue() if updates else PQueueHeap() # NOTE: change this line to set max or min prioritisation
        ...
        if self.oracle:
            ...
            self.log("QualityFrontierManager will use father's quality as priority.")
    ...
    def update(self, page: WebPage, father: WebPage) -> bool:
        ...
        if priority is None:
            priority = self.MIN_PRIORITY
            print(f"Warning: page={url} has no priority, setting it to {priority}.")
    
        return self.queue.update(item=url, ext_priority=priority)
...
def init_frontier_manager(frontier_type: str, updates: bool, verbose= True):
    ...
    elif frontier_type == "quality":
        return QualityFrontierManager(updates=updates, verbose=verbose, oracle=False)
