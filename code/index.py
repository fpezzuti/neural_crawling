...
def main():
    ...
    parser.add_argument("--evaluate", type=bool, default=True, help="Compute evaluation metrics for the generated run.")
    parser.add_argument("--periodic", type=bool, default=True, help="Every limit documents, build and index and compute a run.")
    parser.add_argument("--remove_all", type=bool, default=True, help="Remove all indexes after inferencec.")
    ...
    if args.periodic:
        ...
        while True:
            ...
            sub_exp_name = f"{args.exp_name}/limit_{limit}"

            indexer = Indexer(collection=args.collection, experiment_name=sub_exp_name, downloads_dir=downloads_dir, verbosity=VERBOSITY, num_docs_limit=limit, batch_size=args.batch_size)
            indexer.index()
